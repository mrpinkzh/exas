...
namespace ExAs.Api
{
    ...
    public class DateTimeAssertionFeature
    {
        ...
        public void CloseTo_OnLunchTime_WithLunchTimePlus5Seconds_ShouldPass()
        {
            ...
            var result = dinnerAppointment.Evaluate(da => da.Member(x => x.Time).IsCloseTo(Times.LunchTime(), 5.Seconds()));
            Assert.IsTrue(result.succeeded);
        ...
        {
            var dinnerAppointment = new DinnerAppointment(Times.LunchTime());
            var result = dinnerAppointment.Evaluate(da => da.Member(x => x.Time).IsCloseTo(Times.DinnerTime(), 5.Seconds()));
            Assert.IsFalse(result.succeeded);
